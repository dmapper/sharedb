...
var warnDeprecatedDoc = false;
var warnDeprecatedAfterSubmit = false;
...
Backend.prototype._shimDocAction = function() {
  ...
  this.use(this.MIDDLEWARE_ACTIONS.readSnapshots, function(request, callback) {
    async.each(request.snapshots, function(snapshot, eachCb) {
      var docRequest = {index: request.index, collection: request.collection, id: snapshot.id, snapshot: snapshot};
...
Backend.prototype._sanitizeSnapshots = function(agent, index, projection, collection, snapshots, snapshotType, callback) {
  if (projection) {
    try {
...
Backend.prototype.fetch = function(agent, index, id, callback) {
  ...
  backend.db.getSnapshot(collection, id, fields, null, function(err, snapshot) {
    ...
    backend._sanitizeSnapshots(agent, index, snapshotProjection, collection, snapshots, backend.SNAPSHOT_TYPES.current, function(err) {
      if (err) return callback(err);
...
Backend.prototype._query = function(agent, request, callback) {
  ...
  request.db.query(request.collection, request.query, request.fields, request.options, function(err, snapshots, extra) {
    if (err) return callback(err);
    backend._sanitizeSnapshots(agent, request.index, request.snapshotProjection, request.collection, snapshots, backend.SNAPSHOT_TYPES.current, function(err) {
...
Backend.prototype.fetchSnapshot = function(agent, index, id, version, callback) {
  ...
  this._fetchSnapshot(collection, id, version, function (error, snapshot) {
    ...
    backend._sanitizeSnapshots(agent, index, snapshotProjection, collection, snapshots, snapshotType, function (error) {
      if (error) return callback(error);
